...
import { pickTestOnlyClients, type QuoteOwnerRow } from "@/lib/admin.functions";
...
describe("pickTestOnlyClients", () => {
  ...
  it("keeps only emails whose projects are all test projects", () => {
  ...
  it("never lists an account that holds a staff role", () => {
    const result = pickTestOnlyClients(rows, new Set(["user-staff", "user-test"]), users);
    expect(result).toEqual([]);
  });

  it("reports clients with no sign-in account without a user id", () => {
