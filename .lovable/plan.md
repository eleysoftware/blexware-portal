# Reminder setup: ZeptoMail for email, Textbee for SMS, Brevo for contacts
...
2. **Text messages move to Textbee.dev.** Textbee sends texts through an Android phone you own, running the Textbee app. Brevo only keeps the "BLEXware Clients" contact list, with each client's name, email and mobile number. Brevo does not send any texts.
...
- **Sync clients to Brevo:** when a client is created or edited, their Brevo contact is added or updated on the "BLEXware Clients" list. If the sync fails, it's logged. The invoice still goes out.
...
- New helper `src/lib/brevo.server.ts` handles contacts only, with `POST /v3/contacts` (`updateEnabled: true`, `attributes.SMS`, `listIds`) through the existing Brevo connection.
