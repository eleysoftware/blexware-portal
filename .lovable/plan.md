...
2. **Direct PayPal Business integration.** Server-side PayPal calls to create an order for the validated amount, capture it, read its status, and issue refunds. On the invoice page, the PayPal buttons let a client pay with their PayPal balance or account; where your account is approved for it, a card form is shown for clients who don't have PayPal. Styled to match the current payment card so the client experience looks the same. PayPal webhooks arrive at a new verified endpoint and update invoices exactly the way the Hyperswitch webhook does today.
